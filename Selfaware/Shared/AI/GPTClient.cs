                ...
                List<ChatMessage> messages = new()
             {
                ...
                ChatCompletionOptions options = new()
                {
                    ResponseFormat = ChatResponseFormat.CreateJsonSchemaFormat(
                        ...
                        jsonSchema: BinaryData.FromString(QuizSchemaProvider.GetOpenAiJsonSchemaString()),
                ...
                {
                    return ServiceResult<AiQuizResponseDto>.Failed("OpenAI failed to return any completion choices.");
                }
                ...
                if (questions == null)
                {
                    return ServiceResult<AiQuizResponseDto>.Failed("Failed to parse the OpenAI JSON response.");
                }

                Console.WriteLine($"Deserialazied Generated Quiz {questions.Questions}");
                return ServiceResult<AiQuizResponseDto>.Ok(questions, "Quiz generated successfully via OpenAI");
            }
            catch (Exception ex)
            {
                return ServiceResult<AiQuizResponseDto>.Failed($"System crashed during OpenAI generation: {ex.Message}");
            }
        }
    }
}
