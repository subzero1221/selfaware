...
namespace Selfaware.Shared.AI
{
    public class GeminiClient : IAIClient
    {
        ...
        public GeminiClient(IOptions<GeminiSettings> geminiSettings)
        {
        ...
        )
        {
            try
            {
                ...
                )
                {
                    return ServiceResult<AiQuizResponseDto>.Failed(
                        "AI API failed to return any candidates."
                    );
                }
                ...
                {
                    return ServiceResult<AiQuizResponseDto>.Failed(
                        $"AI returned empty content. Finish Reason: {firstCandidate.FinishReason}"
                    );
                }
                Console.WriteLine("----> CALLING GEMINI API NOW 2... <----");
                var jsonResponse = firstCandidate.Content.Parts[0].Text;
                Console.WriteLine(
                    $"=== RAW TEXT FROM AI ===\n{jsonResponse}\n========================"
                );
            ...
            {
                return ServiceResult<AiQuizResponseDto>.Failed(
                    $"System crashed during AI generation: {ex.Message}"
                );
            }
        }
    }
